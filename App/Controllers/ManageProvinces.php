...
class ManageProvinces extends IController {
    use Readxls;
    /**
     * @var \App\Model\ManageProvinces;
     */
    protected $model;
    ...
    function insert() {
        ...
        $result = $this->invoke('Provinces')->getRow($_POST['year'], $_POST['month']);
        if($result)
            $this->error(2001, '插入失败,该记录已经存在');
        $data = $this->_file('reception_number');
        ...
        $reception = json_encode($reception, JSON_UNESCAPED_UNICODE);
        $data = [
            'year' => $_POST['year'],
            'month' => $_POST['month'],
    ...
    function receptionNumberUpdate() {
        $data = $this->_file('reception_number');
        $rows = $this->getData($data['path']);
        foreach ($rows as $row) {
            $reception[] = [
        ...
        $reception = json_encode($reception, JSON_UNESCAPED_UNICODE);
        $result = $this->model->receptionNumberUpdate(intval($_POST['id']), $reception);
